...
//        }
	...
	testHelpers.devOnlyTest("Logging: Helper not found in stache template(#726)", function () {
	...
	testHelpers.devOnlyTest("Logging: Variable not found in stache template (#720)", function () {
		var teardown = testHelpers.willWarn('can-stache/src/expression.js: Unable to find key or helper "user.name".');
	...
	testHelpers.dev.devOnlyTest("warn on missmatched tag (canjs/canjs#1476)", function() {
		var teardown,
			overwrite = function() {
				teardown = testHelpers.willWarn("filename.stache:3: unexpected closing tag {{/foo}} expected {{/if}}");
			};
